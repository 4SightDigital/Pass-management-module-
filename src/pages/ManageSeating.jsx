...
const ManageSeating = () => {
  ...
  const getVenueTotalSeats = (venue) => {
    return venue.seating?.reduce((total, cat) => total + (cat.categoryTotalSeats || 0), 0) || 0;
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 p-4 md:p-6">
      ...
      <div className="mb-8">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <div>
            <h1 className="text-3xl font-bold text-gray-800 mb-2">Manage Seating Layout</h1>
            ...
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-800">
                  {venues.reduce((total, venue) => total + getVenueTotalSeats(venue), 0)}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-200">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-800">
                  {venues.reduce((total, venue) => total + (venue.seating?.length || 0), 0)}
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="space-y-4">
        {venues.length === 0 ? (
          <div className="bg-white rounded-2xl p-8 text-center shadow-sm border border-gray-200">
            ...
            <h3 className="text-xl font-semibold text-gray-800 mb-2">No Venues Added</h3>
            <p className="text-gray-600 mb-6">Add venues first to configure their seating layout</p>
          </div>
        ) : (
                ...
                >
                  ...
                  <div className="flex items-center space-x-6">
                    ...
                    <div className="text-right hidden md:block">
                      <div className="text-sm text-gray-500">Seat Configuration</div>
                      ...
                      <div className="w-32 h-2 bg-gray-200 rounded-full overflow-hidden mt-1">
                        <div 
                          ...
                          style={{ width: `${venueCapacity ? (venueTotalSeats / venueCapacity * 100) : 0}%` }}
                        />
                      </div>
                    </div>
                  ...
                  <div className="px-6 pb-6 pt-2 border-t border-gray-200">
                    ...
                    <div className="mb-6 p-4 bg-gradient-to-r from-emerald-50 to-blue-50 rounded-xl">
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div>
                          <h4 className="font-medium text-gray-800 mb-1">Venue Capacity Overview</h4>
                          <div className="flex items-center space-x-4">
                            <div className="flex items-center">
                              <div className="w-3 h-3 bg-emerald-500 rounded-full mr-2"></div>
                              <span className="text-sm text-gray-600">Configured: {venueTotalSeats} seats</span>
                            </div>
                            <div className="flex items-center">
                              <div className="w-3 h-3 bg-blue-500 rounded-full mr-2"></div>
                              <span className="text-sm text-gray-600">Total Capacity: {venueCapacity} seats</span>
                            </div>
                          </div>
                        </div>
                        {venueCapacity > 0 && (
                          <div className="text-right">
                            <div className="text-2xl font-bold text-gray-800">
                              {Math.round((venueTotalSeats / venueCapacity) * 100)}%
                            </div>
                            <div className="text-sm text-gray-500">Capacity Utilized</div>
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="mb-6">
                      <div className="flex items-center justify-between mb-4">
                        <h4 className="text-lg font-semibold text-gray-800">Seating Categories</h4>
                        <span className="text-sm text-gray-500">
                          {venue.seating?.length || 0} categorie{venue.seating?.length !== 1 ? 's' : ''}
                        </span>
                      </div>
                      <AddCategory
                        onAdd={(name, seats) => {
                    ...
                    <div className="space-y-4">
                      {venue.seating?.length === 0 ? (
                        ...
                        venue.seating?.map((cat, catIndex) => {
                          const isCategoryExpanded = expandedCategories[`${venue.id}-${cat.id}`];
                          const usedSeats = getCategoryUsedSeats(cat.subCategories);
                          const availableSeats = cat.categoryTotalSeats - usedSeats;
                            ...
                            >
                              ...
                              <div className="flex justify-between items-start mb-4">
                                <div className="flex items-center space-x-3">
                                  ...
                                  <div>
                                    <h5 className="font-bold text-gray-800">{cat.categoryName}</h5>
                                    <div className="flex items-center space-x-4 mt-1">
                                      <span className="text-sm text-gray-600">Total: {cat.categoryTotalSeats} seats</span>
                                      <span className={`text-sm px-2 py-1 rounded-md ${
                                        availableSeats >= 0 
                                          ? 'bg-emerald-100 text-emerald-800' 
                                          : 'bg-red-100 text-red-800'
                                      }`}>
                                        {availableSeats >= 0 ? `${availableSeats} available` : `${-availableSeats} overflow`}
                                      </span>
                                    </div>
                                  </div>
                                </div>
                                <button
                                  onClick={() => toggleCategory(venue.id, cat.id)}
                                  className="p-2 hover:bg-gray-200 rounded-lg transition-colors"
                                >
                                  <svg className={`w-5 h-5 text-gray-500 transition-transform ${isCategoryExpanded ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                                  </svg>
                                </button>
                              </div>
                              ...
                              {cat.categoryTotalSeats > 0 && (
                                <div className="mb-4">
                                  ...
                                  <div className="w-full h-2 bg-gray-200 rounded-full overflow-hidden">
                                    <div 
                                      className="h-full bg-gradient-to-r from-emerald-500 to-blue-500 transition-all duration-500"
                                      style={{ width: `${(usedSeats / cat.categoryTotalSeats) * 100}%` }}
                                    />
                                  </div>
                                </div>
                              )}
                              ...
                              {/* Subcategory List */}
                              {isCategoryExpanded && cat.subCategories.length > 0 && (
                                <div className="mt-4">
                                  <div className="flex items-center justify-between mb-3">
                                    <h6 className="font-medium text-gray-700">Subcategories</h6>
                                    <span className="text-sm text-gray-500">{cat.subCategories.length} subcategorie{cat.subCategories.length !== 1 ? 's' : ''}</span>
                                  </div>
                                          ...
                                          <div className="flex items-center space-x-3">
                                            ...
                                            <div>
                                              ...
                                              <div className="text-sm text-gray-600 mt-1">
                                                Price: ${s.price || '0'} • Seats: {s.seats}
                                              </div>
                                            </div>
                                          </div>
                                          <div className="text-right">
                                            <div className="text-lg font-bold text-emerald-600">{s.seats}</div>
